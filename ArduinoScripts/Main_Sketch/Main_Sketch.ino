...
void Training_Stage_0() {
  ...
  Water_Process0(); // Start Water On 2 Vavles For Water_Duration (In Order).
  ...
  while (1) {
    ...
    if (millis() - Current_Time >= 2000) {//Send Message Every 1 Sec.
	  Lick_In_Response_Time = 0;
      Current_Time = millis();
      GUI_Recieve(); // Send Message To GUI.
    }
	...
	if (WaterFlag == 1) 
			Water_Process2(); // Start Water On 2 Valves For Water_Duration (Multitasking Process).
...
void Training_Stage_3() {
  ...
  for (Actual_Trials = 1; Actual_Trials <= Number_Of_Trials; Actual_Trials++) {
    ...
    while (millis() - Current_Time <= Response_Time) {
      ...
      case 1: {
        WaterFlag = 1;
		if (Fisrt_Lick_In_Response_Time)
			Current_Time_Water = millis();
        Lick_At_Correct_Port = 1;
        GUI_Recieve(); //Send Message After Lick.
        break;
...
//Water_Process2 - Start Water Open 2 Valves (Multitasking Process):
void Water_Process2() {
...
//Water_Process0 - Start Water Open 2 Valves (In Order):
void Water_Process0() {
