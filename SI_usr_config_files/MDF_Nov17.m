...
objectiveResolution = 73.0769;           % Resolution of the objective in microns/degree of scan angle
...
beamDaqs(1).displayNames = {'QubigBeam'};                  % Optional string cell array of identifiers for each beam
beamDaqs(1).voltageRanges = 1.8;                % Scalar or array of values specifying voltage range to use for each beam. Scalar applies to each beam.
...
beamDaqs(1).calOffsets = -7.05595e-05;                    % Array of beam calibration offset voltages for each beam calibration channel
...
motors(1).comPort = 1;                  % Integer identifying COM port for controller, if using serial communication
...
externalSampleClockRate = 80.3e6;       % [Hz]: nominal frequency of the external sample clock connected to the CLK IN terminal (e.g. 80e6); actual rate is measured on FPGA
